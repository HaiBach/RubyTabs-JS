...
class RubyDOM {
  ...
  constructor(selector, attrs) {
    ...
    // Case: selector is One Node
      ...
      if( /^<.+>$/.test(selector) ) {
        ...
        let $absNode = document.createElement('div');
        $absNode.innerHTML = selector;
        let $node = $absNode.childNodes[0];
  ...
  // Remove Data
  removeData(name) {
    let arrName = RubyM.RemoveWhiteSpace(name).split(' ');
  ...
  addClass(str) {
    ...
    for( let i = 0, len = this.$items.length; i < len; i++ ) {
      let $itemCur = this.$items[i];

      // let strClass = this.attr('class'),
      let strClass = $itemCur.getAttribute('class');
      let arrClass = RubyM.RemoveWhiteSpace(strClass).split(' '),
          isChange = false;

       for( let j = 0, lenJ = arrStr.length; j < len; j++ ) {
         let valueCur = arrStr[j],
  ...
  find(selector) {
    ...
    for( let i, len = this.$items.length; i < len; i++ ) {
      let $childsOfItem = this.$items[i].querySelectorAll(selector);
  ...
  children(selector) {
    ...
    for( let i in this.$items ) {
      ...
      if( selector !== undefined ) {
        let $childsByQuery = this.$items[i].querySelectorAll(selector);
        ...
        for( let x = 0, lenX = $childsByQuery.length; x < lenX; x++ ) {
          ...
          for( let y = 0, lenY = $childsOfItem.length; y < lenY; y++ ) {
    ...
    let $childs = (selector === undefined) ? $childs1 : $childs2;
    return new RubyDOM($childs);
  }

  // Wrap Inner
  wrapInner($wrap) {
    let $itemCur = this.$items[0];
    let orgHTML = $itemCur.innerHTML;
...
window[rt01VA.rubyName + 'Main'] = function($ruby, OptsJS) {
  ...
  M = $.extend(true, {}, rt01VA.M, {
    ...
    Data : function($obj, opts) {
      ...
      if( !($obj instanceof RubyDOM) ) return false;
  ...
  RENDER = {
    ...
    Canvas : function() {
       ...
       */
        ...
        if( $slides.length ) {
          ...
          if( tagCanvas == 'div' && $slides.$items[0].tagName.toLowerCase() == 'li' ) tagCanvas = 'ul';
    ...
    Slide : function($sl) {
      var slClass    = va.ns + o.nameSlide,
          slTagBegin = $sl.$items[0].tagName.toLowerCase();
      ...
      // Slide without wrapper, only one element link '<a>', '<img>'
        ...
        var canvasTag = M.Data($canvas)['tagName'],
            ...
            slTag     = (canvasTag == 'ul') ? '<li></li>' : '<div></div>',
            $wrapper  = new RubyDOM(slTag, { 'class': slClass });
    ...
    CheckMobileTap : function($items) {
      ...
      $items.each(function() {
        ...
        $item.on(va.ev.swipe.start, function(e) {
          var itemData = M.Data( new RubyDOM(this) ),
