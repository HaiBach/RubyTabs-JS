...
var RubyDOM = /** @class */ (function () {
    // Class Constructor
    function RubyDOM(selector, attrs) {
        var $items;
        // Case: Create new Empty Element without 'selector' variable
        if (selector === undefined) {
            ...
            $items = selector;
            this._ConvertNode($items);
        }
        else if (Object.prototype.toString.call(selector) === '[object NodeList]') {
            $items = selector;
            this._ConvertNode($items);
        }
        else if (RubyM.IsString(selector)) {
            // Case: Create new Element by '<ele>'
            if (/^<.+>$/.test(selector)) {
                // Create abstract Node and detect node is children
                var $absNode = document.createElement('div');
                $absNode.innerHTML = selector;
                var $node = $absNode.childNodes[0];
                // Add Attribute for new element
                if (RubyM.IsPlainObject(attrs)) {
                    for (var key in attrs) {
                        $node.setAttribute(key, attrs[key]);
                    }
                }
                // Convert node to array
                this._ConvertNode([$node]);
            }
            else {
    ...
    RubyDOM.prototype._ConvertNode = function ($items) {
        this.$items = [];
    ...
    RubyDOM.prototype.data = function (name, value) {
        ...
        if (value === undefined) {
            ...
            if (data === undefined) {
                var valueGet = $item.getAttribute(name);
                // Convert type value if is 'string'
                if (RubyM.IsString(valueGet))
                    valueGet = RubyM.StringToJson(valueGet);
                return valueGet;
            }
            else
                return data;
    ...
    // Remove Data
    RubyDOM.prototype.removeData = function (name) {
        var arrName = RubyM.RemoveWhiteSpace(name).split(' ');
        // Loop: support remove many data on 'name' variable
    ...
    RubyDOM.prototype.attr = function (name, value) {
        var $item = this.$items[0];
        // GETTER
        if (value === undefined) {
    ...
    // Remove Attribute
    RubyDOM.prototype.removeAttr = function (name) {
        var arrName = RubyM.RemoveWhiteSpace(name).split(' ');
        // Loop: support remove many atrribute on 'name' variable
        for (var i = 0, len = arrName.length; i < len; i++) {
            this.attr(arrName[i], '');
        }
        return this;
    ...
    // Modify Style CSS
    RubyDOM.prototype.css = function (name, value) {
        // Case: 'name' is 'String'
        if (RubyM.IsString(name)) {
            for (var i = 0, len = this.$items.length; i < len; i++) {
                this.$items[i]['style'][name] = value;
            }
        }
        else if (RubyM.IsPlainObject(name)) {
            for (var i = 0, len = this.$items.length; i < len; i++) {
                for (var key in name) {
                    this.$items[i]['style'][key] = name[key];
                }
            }
        }
        return this;
    ...
    // Add Class
    RubyDOM.prototype.addClass = function (str) {
        var arrStr = RubyM.RemoveWhiteSpace(str).split(' ');
        // Loop for all Elements in this.$items
        for (var i = 0, len = this.$items.length; i < len; i++) {
            var $itemCur = this.$items[i];
            // let strClass = this.attr('class'),
            var strClass = $itemCur.getAttribute('class');
            var arrClass = RubyM.RemoveWhiteSpace(strClass).split(' '), isChange = false;
            for (var j = 0, lenJ = arrStr.length; j < len; j++) {
                var valueCur = arrStr[j], index = arrClass.indexOf(valueCur);
    ...
    // Remove Class
    RubyDOM.prototype.removeClass = function (str) {
        var arrStr = RubyM.RemoveWhiteSpace(str).split(' ');
        // Loop for all Elements in this.$items
        for (var i = 0, len = this.$items.length; i < len; i++) {
            var $itemCur = this.$items[i];
    ...
    // Find Elements
    RubyDOM.prototype.find = function (selector) {
        var $childs = [];
        // Find Children of Items
        for (var i = void 0, len = this.$items.length; i < len; i++) {
            var $childsOfItem = this.$items[i].querySelectorAll(selector);
            // Case: var 'selector' not exist
            for (var j = 0, lenJ = $childsOfItem.length; j < lenJ; j++) {
                $childs.push($childsOfItem[j]);
            }
        }
        return $childs.length ? new RubyDOM($childs)
            : new RubyDOM();
    };
    // Search Children
    RubyDOM.prototype.children = function (selector) {
        var $childs1 = [];
        var $childs2 = [];
        // Find Children of Items
        for (var i in this.$items) {
            var $childsOfItem = this.$items[i].children;
            // Case: var 'selector' not exist
            for (var j = 0, lenJ = $childsOfItem.length; j < lenJ; j++) {
                $childs1.push($childsOfItem[j]);
            }
            // Case: var 'selector' is exist
            if (selector !== undefined) {
                var $childsByQuery = this.$items[i].querySelectorAll(selector);
                // Find by Query
                for (var x = 0, lenX = $childsByQuery.length; x < lenX; x++) {
                    var isExist = false;
                    // Compare Node by Query with Node 'children'
                    for (var y = 0, lenY = $childsOfItem.length; y < lenY; y++) {
        ...
        var $childs = (selector === undefined) ? $childs1 : $childs2;
        return new RubyDOM($childs);
    };
    // Wrap Inner
    RubyDOM.prototype.wrapInner = function ($wrap) {
        var $itemCur = this.$items[0];
        var orgHTML = $itemCur.innerHTML;
        // Add childNode into $wrap
        $wrap.$items[0].innerHTML = orgHTML;
        // Clear all node on $itemCur
        $itemCur.innerHTML = '';
        // Append $wrap into $itemCur
        $itemCur.appendChild($wrap.$items[0]);
    };
    RubyDOM.prototype.append = function ($ele) {
        for (var i = 0, len = this.$items.length; i < len; i++) {
            this.$items[i].appendChild($ele.$items[0]);
        }
        return this;
    };
    // Remove Elements
    RubyDOM.prototype.remove = function () {
        for (var i = 0, len = this.$items.length; i < len; i++) {
            var $itemCur = this.$items[i];
            $itemCur.parentElement.removeChild($itemCur);
        }
    };
    // Each
    RubyDOM.prototype.each = function (func) {
        for (var i = 0, len = this.$items.length; i < len; i++) {
            // Pass 'this' variable && Excute function
            func.call(this.$items[i], i);
        }
    };
    RubyDOM.prototype.eq = function (id) {
        return new RubyDOM(this.$items[id]);
    };
    // Compare Node
    RubyDOM.prototype.is = function ($node) {
        return this.$items[0].isEqualNode($node.$items[0]);
    };
    // Add element to Array
    RubyDOM.prototype.add = function ($nodes) {
        // Check is RubyDOM
        if ($nodes instanceof RubyDOM) {
            for (var i = 0, len = $nodes.length; i < len; i++) {
                this.$items.push($nodes.$items[i]);
            }
            // Update variable
 ...
 */
var RubyM = /** @class */ (function () {
    function RubyM() {
    }
    /**
     * DISPLAY ERROR MESSAGES
     */
    RubyM.Message = function (message, detail) {
     ...
     */
    RubyM.StringToJson = function (str, messageError) {
     ...
     */
    RubyM.JsonToString = function (json, messageError) {
     ...
     * Covert to CamelCase
     */
    RubyM.CamelCase = function (prop) {
        return prop.replace(/-([a-z])/gi, function (m, prop) {
            return prop.toUpperCase();
        });
    };
    RubyM.RemoveWhiteSpace = function (str) {
    ...
    RubyM.PInt = function (v) { return /^\-?\d+/g.test(v) ? parseInt(v, 10) : 0; };
    // Check element is Plain Object
    RubyM.IsPlainObject = function (v) { return Object.prototype.toString.call(v) === '[object Object]'; };
    // Check element is Empty Object
    RubyM.IsEmptyObject = function (v) { return RubyM.IsPlainObject(v) && RubyM.JsonToString(v) === '{}'; };
    // Check element is Array
    RubyM.IsArray = function (v) { return Object.prototype.toString.call(v) === '[object Array]'; };
    // Check element is Number
    RubyM.IsNumber = function (n) { return !isNaN(parseFloat(n)); };
    // Check element is String
    RubyM.IsString = function (v) { return Object.prototype.toString.call(v) === '[object String]'; };
    /**
     * ASSIGN 2 OR MORE OBJECT INTO ONE OBJECT
     */
    RubyM.Assign = function () {
        var args = arguments, objNew;
     ...
     */
    RubyM.SizeNoTransform = function ($el, type, isMargin) {
        /**
    ...
    RubyM.Width = function ($el) { return this.SizeNoTransform($el, 'Width'); };
    RubyM.Height = function ($el) { return this.SizeNoTransform($el, 'Height'); };
    RubyM.InnerWidth = function ($el) { return this.SizeNoTransform($el, 'InnerWidth'); };
    RubyM.InnerHeight = function ($el) { return this.SizeNoTransform($el, 'InnerHeight'); };
    RubyM.OuterWidth = function ($el, isMargin) { return this.SizeNoTransform($el, 'OuterWidth', isMargin); };
    RubyM.OuterHeight = function ($el, isMargin) { return this.SizeNoTransform($el, 'OuterHeight', isMargin); };
    /**
     * METHODS RELATE TO MATH
     */
    RubyM.A = function (v) { return Math.abs(v); };
    RubyM.R = function (v) { return Math.round(v); };
    RubyM.C = function (v) { return Math.ceil(v); };
    RubyM.Ra = function () { return Math.random(); };
    RubyM.Rm = function (m, n) { return M.Ra() * (n - m) + m; };
    RubyM.Sum = function (a, to) {
        var total = 0;
        if (to < 0)
            return total;
    ...
    RubyM.PPercent = function (v, source) {
        if (v > 0 && v < 1)
            v *= source;
        return M.R(v);
    };
    // Parse the value have percent unit to Pixel unit
    RubyM.PercentToPixel = function (value, source) {
        var result = null;
    ...
    RubyM.PStyleToJson = function ($obj) {
    ...
    RubyM.ElesIsNumber = function (arr, lenCheck) {
        var len = arr.length, isNum = RubyM.IsArray(arr) && len === lenCheck;
        if (isNum) {
            for (var i = 0; i < len; i++) {
     ...
     */
    RubyM.Shift = function ($obj, isShift) { return isShift ? $obj.shift() : $obj.pop(); };
    RubyM.Push = function ($obj, v, isPush) { return isPush ? $obj.push(v) : $obj.unshift(v); };
    /**
     * RANDOM EFFECT IN THE EFFECT ARRAY[]
     */
    RubyM.RandomInArray = function (arr, except) {
        ...
        if (RubyM.IsArray(arr)) {
             ...
             */
            var itemCur = RubyM.Assign([], arr), indexItemLast = $.inArray(except, itemCur);
    ...
    RubyM.RandomInArray2 = function (arrSource, arrCopy, except) {
     ...
     */
    // Swipe swap variable
    RubyM.SwapVaOnSwipe = function () { return va.$swipeCur.is($canvas) ? va.can : va.pag; };
    // Toggle add/removeClass on object
    RubyM.XClass = function ($obj, isAdd, str) { $obj[(isAdd ? 'add' : 'remove') + 'Class'](str); };
    // Capitalize the first letter of sting
    RubyM.ProperCase = function (str) { return str.charAt(0).toUpperCase() + str.slice(1); };
    // Convert '{ns}' to namespace
    RubyM.NS = function (str) {
     ...
     */
    RubyM.MatchMedia = function (min, max, isWidthOfRuby) {
        /**
     ...
     */
    RubyM.GetValueInRange = function (value, valueName) {
    ...
    RubyM.GetIndexInResponsive = function (resLevels) {
        var index = null;
    ...
    RubyM.ParseGrid = function (fromValue, isInherit) {
     ...
     *  + Remove nested ruby
     */
    RubyM.Find = function ($target, selector) {
...
(function () {
    ...
    window[rt01VA.rubyName + 'Main'] = function ($ruby, OptsJS) {
        ...
        PROP = {
            ...
            Layout: function () {
                ...
                if (o.fx == 'line')
                    va.fxLayout = 'line';
                else if ($.inArray(o.fx, va.fxInLayoutDot) !== -1 || $.isArray(o.fx))
                    va.fxLayout = 'dot';
            ...
            Pagination: function () {
                ...
                if (RubyM.IsNumber(op.width) || RubyM.IsNumber(op.height))
            ...
            Slides: function () {
                ...
                va.$s.each(function (i) {
                    var $slCur = new RubyDOM(this), slData = M.Data($slCur), optsCur = slData['opts'] || {};
                    /**
                     * SETUP REQUIRED PART
        ...
        RENDER = {
            ...
            Structure: function () {
                ...
                va.$s = new RubyDOM();
                $canvas.children().each(function () { RENDER.Slide(new RubyDOM(this)); });
                // Setup each elements each slide
                va.$s.each(function () {
                    var $slCur = new RubyDOM(this);
            ...
            Anchor: function () {
                ...
                if (is.fullwidth || is.fullscreen) {
                    ...
                    va.$anchor = new RubyDOM('<div></div>', { 'class': classAnchor });
            ...
            Viewport: function () {
                ...
                if (viewport.length)
                    ...
                    $ruby.wrapInner(new RubyDOM('<div></div>', { 'class': viewClass }));
            ...
            Canvas: function () {
                 ...
                 */
                if (canvas.length) {
                    tagCanvas = canvas.$items[0].tagName.toLowerCase();
                }
                else {
                    /**
                     * CASE: SLIDES EXIST
                    ...
                    if ($slides.length) {
                        ...
                        if (tagCanvas == 'div' && $slides.$items[0].tagName.toLowerCase() == 'li')
                            tagCanvas = 'ul';
                        //- var html = (tagCanvas == 'ul') ? '<ul/>' : divdiv;
                        //- $slides.wrapAll( $(html, {'class': canvasClass}) );
                        var html = (tagCanvas == 'ul') ? '<ul></ul>' : '<div></div>';
                        $viewport.wrapInner(new RubyDOM(html, { 'class': canvasClass }));
                    }
                    else {
                        $viewport.append(new RubyDOM('<div></div>', { 'class': canvasClass }));
                    }
                }
                /**
            ...
            OverlayGhost: function ($parent) {
                var $overlayGhost = new RubyDOM('<div></div>', { 'class': va.ns + 'overlay-ghost' });
             ...
             */
            Slide: function ($sl) {
                var slClass = va.ns + o.nameSlide, slTagBegin = $sl.$items[0].tagName.toLowerCase();
                /**
                 * SETUP OF TAG NAME
                 */
                ...
                if (/^(div|li|article|section)$/.test(slTagBegin) || $sl.hasClass(slClass)) {
                    ...
                    slTag = (canvasTag == 'ul') ? '<li></li>' : '<div></div>', $wrapper = new RubyDOM(slTag, { 'class': slClass });
                    $sl.wrap($wrapper);
            ...
            CapPagHTML: function ($slCur) {
                ...
                if (!$pagItem.length) {
                    $pagItem = new RubyDOM('<div></div>', { 'class': ns + 'pagitem' });
            ...
            SearchNode: function (classSearch) {
                var $dom = new RubyDOM(), NAME = va.name;
                /**
                 * CONDITIONAL EXECUTION: MUST BE 'NAME' DEFINED
                 */
                if (NAME !== null && NAME !== undefined) {
                    new RubyDOM(classSearch).each(function () {
                        var $item = new RubyDOM(this), markupData = $item.data(rt01VA.rubyName + 'Markup');
                ...
                // Return object found
                return $findNext.length ? $findNext : new RubyDOM();
            },
            /**
            ...
            Into: function (intoParent, $child) {
                ...
                switch (intoParent) {
                    ...
                    case 'nav':
                        if (!va.$nav) {
                            va.$nav = new RubyDOM('<div></div>', { 'class': va.ns + o.nameNav });
            ...
            DivImg: function (name, parent, isAfter) {
                ...
                if (o['is' + nameUpper]) {
                    if (!va[name].length) {
                        ...
                        isAfter && parent.after(new RubyDOM(tag)) || parent.before(new RubyDOM(tag));
                    }
                }
                else if (va[name].length) {
                    console.log('#', name, va[name], va);
                    va[name].remove();
                }
            },
            /**
        ...
        SIZE = {
            ...
            HeightFixedForRuby: function () {
                ...
                if (is.fullscreen) {
                     ...
                     */
                    if (o.offsetBy !== null) {
                        var $offset = new RubyDOM(o.offsetBy);
                        // Height Ruby will substract by height offsetBy container
                        $offset.each(function () {
                            hWin -= M.OuterHeight(new RubyDOM(this), true);
                        });
        ...
        EVENTS = {
            ...
            CheckMobileTap: function ($items) {
                $items.each(function () {
                    var $item = new RubyDOM(this);
                    // Register event swipe start
                    $item.on(va.ev.swipe.start, function (e) {
                        var itemData = M.Data(new RubyDOM(this)), i = EVENTS.GetEventRight(e);
        ...
        API = {
            ...
            updateOnSlides: function (options) {
                if (!RubyM.IsPlainObject(options))
                    return;
    ...
    window[rt01VA.rubyName] = function ($items, opts, value) {
         ...
         */
        $items.each(function () {
            var $self = new RubyDOM(this), ruby = $self.data(rubyName);
    ...
    rt01MODULE.AUTOINIT = function ($ruby) {
        $ruby.each(function (i) {
            var $self = new RubyDOM(this), data = $self.data(rt01VA.rubyData) || {}, rubyName = rt01VA.rubyName, isJson = true;
            /**
             * CHECK RUBY INITIALIZATION
             *  + Remove automatically initialize for ruby
             */
            if (RubyM.IsPlainObject(data) && RubyM.IsEmptyObject(data))
                return;
            /**
